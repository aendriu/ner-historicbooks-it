...
RESULTS_FILE = os.path.join(TEST_DIR, "evaluations", "results", "ner_chunking_evaluation_results.json")

# Tolleranza: ±1 Blocco (essendo ogni blocco da 2000 caratteri, 1 blocco di scarto è già molto ampio)
TOLERANCE_WINDOW = 1
MIN_PRECISION = 0.40
MIN_RECALL    = 0.40
...
def test_ner_chunking_boundaries():
    if not os.path.exists(RESULTS_FILE):
        pytest.skip(
            f"Gold standard non trovato: {RESULTS_FILE}\n"
            "Esegui prima: python3 scripts/generate_ner_chunking_gold.py"
        )
    ...
    print(f"\n{'='*35}")
    print(f"  RISULTATI CHUNKING NER")
    print(f"{'='*35}")
    print(f"  Libro analizzato           : {results.get('book_name')}")
    print(f"  Blocchi testati            : {results.get('blocks_tested')}")
    print(f"  Tolleranza                 : ±{TOLERANCE_WINDOW} blocchi")
    print(f"  Cambi Scena Gold (Claude)  : {len(gold)}")
    print(f"  Cambi Scena Local (NER)    : {len(local)}")
